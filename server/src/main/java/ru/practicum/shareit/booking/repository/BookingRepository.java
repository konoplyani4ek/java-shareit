...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b WHERE b.item.owner.id = :ownerId ORDER BY b.startDateTime DESC")
    List<Booking> findByOwnerUserId(@Param("ownerId") Long ownerId);

    @Query("SELECT b FROM Booking b WHERE b.booker.id = :userId " +
            "AND b.startDateTime <= :now AND b.endDateTime >= :now ORDER BY b.startDateTime DESC")
    ...
    @Query("SELECT b FROM Booking b WHERE b.booker.id = :bookerId " +
            "AND b.endDateTime < :now ORDER BY b.startDateTime DESC")
    List<Booking> findPastByBookerId(@Param("bookerId") Long bookerId, @Param("now") LocalDateTime now);

    @Query("SELECT b FROM Booking b WHERE b.booker.id = :bookerId " +
            "AND b.startDateTime > :now ORDER BY b.startDateTime DESC")
    ...
    @Query("SELECT b FROM Booking b WHERE b.item.owner.id = :ownerId " +
            "AND b.status = :status ORDER BY b.startDateTime DESC")
    List<Booking> findByOwnerIdAndStatusOrderByStartDateTimeDesc(@Param("ownerId") Long ownerId,
                                                                  @Param("status") Status status);

    boolean existsByBookerIdAndItemIdAndStatusAndEndDateTimeBefore(
